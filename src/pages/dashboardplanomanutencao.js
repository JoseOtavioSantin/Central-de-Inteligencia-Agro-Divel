// Extracted from Dashboardplanomanutencao.html
import { initializeApp } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-app.js";
  ...
  const tipoChecklist = new URLSearchParams(window.location.search).get("tipo") || "checklistPlanoManutencao";
...
function aplicarFiltros() {
  ...
  painel.innerHTML += `
    <tr class="${statusClass}">
      ...
      <td class="acoes">
        ${
          d.statusChecklist === "Concluído"
            ? ""
            : `<button class="btn-acao" data-id="${d.id}" title="Abrir ações">➕</button>`
        }
      </td>
    </tr>`;
  });
  ...
  const municipio = dadosOriginais.municipio || "";
  const modelo = dadosOriginais.modelo || "";
  const fazenda = dadosOriginais.nomeFazenda || "";
  const filial = dadosOriginais.filial || "";
  ...
  const valorNegociado = parseFloat(document.getElementById("valorNegociado").value) || 0;
